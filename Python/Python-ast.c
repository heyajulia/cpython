...
add_ast_annotations(struct ast_state *state)
{
    ...
    Py_DECREF(While_annotations);
    PyObject *Until_annotations = PyDict_New();
    if (!Until_annotations) return 0;
    {
        PyObject *type = state->expr_type;
        Py_INCREF(type);
        cond = PyDict_SetItemString(Until_annotations, "test", type) == 0;
        Py_DECREF(type);
        if (!cond) {
            Py_DECREF(Until_annotations);
            return 0;
        }
    }
    {
        ...
        cond = PyDict_SetItemString(Until_annotations, "body", type) == 0;
        Py_DECREF(type);
        if (!cond) {
            Py_DECREF(Until_annotations);
            return 0;
    ...
    }
    {
        ...
        cond = PyDict_SetItemString(Until_annotations, "orelse", type) == 0;
        Py_DECREF(type);
        if (!cond) {
            Py_DECREF(Until_annotations);
            return 0;
    ...
    cond = PyObject_SetAttrString(state->Until_type, "_field_types",
                                  Until_annotations) == 0;
    if (!cond) {
        Py_DECREF(Until_annotations);
        return 0;
    }
    cond = PyObject_SetAttrString(state->Until_type, "__annotations__",
                                  Until_annotations) == 0;
    if (!cond) {
        Py_DECREF(Until_annotations);
        return 0;
